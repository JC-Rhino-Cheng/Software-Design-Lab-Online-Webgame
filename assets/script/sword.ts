...
export default class sword extends cc.Component {
    ...
    private attack: number = 100;

    private cool_time: number = 0.33;
    private is_weapon_able_to_use: boolean = true;

    @property({type:cc.AudioClip})
        sword_sound: cc.AudioClip = null;
    ...
    start () {
        cc.find('Canvas/inner_map').on(cc.Node.EventType.MOUSE_DOWN, function(event){
            ...
            if(this.is_weapon_able_to_use){
                ...
                cc.audioEngine.playEffect(this.sword_sound, false);
    ...
    AttackTheScope(){
        ...
        for(var i=0;i<children.length;i++){
            ...
            if(this.direction == 0){
                if(mon_map_x >= player_map_x - 1 && mon_map_x <= player_map_x + 1 && mon_map_y >= player_map_y && mon_map_y <= player_map_y + 1)
                    this.Attack(children[i]);
            }else if(this.direction == 1){
                if(mon_map_x >= player_map_x - 1 && mon_map_x <= player_map_x && mon_map_y >= player_map_y - 1 && mon_map_y <= player_map_y + 1)
                    this.Attack(children[i]);
            }else if(this.direction == 2){
                if(mon_map_x >= player_map_x - 1 && mon_map_x <= player_map_x + 1 && mon_map_y >= player_map_y && mon_map_y <= player_map_y + 1)
                    this.Attack(children[i]);
            }else if(this.direction == 3){
                if(mon_map_x >= player_map_x && mon_map_x <= player_map_x + 1 && mon_map_y >= player_map_y - 1 && mon_map_y <= player_map_y + 1)
                    this.Attack(children[i]);
            }
    ...
    play_anim(player_x, player_y){
        if(this.direction == 0){
            ...
            this.node.y = player_y + 60 - 507;
        } else if(this.direction == 1){
            this.node.x = player_x - 60 - 747;
